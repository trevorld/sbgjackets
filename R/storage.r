...
#' @rdname sbgj_storage
#' @export
sbgj_storage_all <- function(output = NULL, ...,
                              paper = c("letter", "a4"),
                              instructions = TRUE) {
    check_dots_empty()
    ...
    bm <- data.frame(title = c("Dice", "Glass Stones", "Pawns", "Polyhedral Dice"),
                     page = c(1L, 3L, 5L, 7L))
...
#' @rdname sbgj_storage
#' @export
sbgj_dice <- function(output = NULL, ...,
                      paper = c("letter", "a4"),
                      instructions = FALSE) {
    check_dots_empty()
    assert_runtime_dependencies()
    ...
    if (!file.exists(pic))
        download.file("https://upload.wikimedia.org/wikipedia/commons/thumb/2/21/Mannen_spelen_dobbelspel%2C_op_de_grond_zit_aap_met_speelkaarten_Titelpagina_voor_De_alea_libri_duo%2C_Amsterdam_1642_De_alea_libri_duo_%28titel_op_object%29%2C_RP-P-1878-A-819.jpg/1193px-thumbnail.jpg",
        pic)
    ...
    #                       rank = rep(1:6, 4L),
    #                       suit = rep(6:1, 4L))
    df <- ppdf::dice_dice(x = rep(1:4 - 0.5, 7L),
                          y = rep((6/7) * (7:1 - 0.5) + 0.1, each = 4L),
                          rank = rep(1:6, length.out = 28L),
                          suit = rep(6:1, length.out = 28L))
    dice <- piecepackr::pmap_piece(df, piecepackr::pieceGrob,
                                   scale = 1.1, envir = envir,
    ...
    credits <- c("* *Mannen spelen dobbelspel, op de grond zit aap met speelkaarten.* (Men playing dice, monkey sitting on the ground with playing cards) by Cornelis van Dalen",
                 "",
...
#' @rdname sbgj_storage
#' @export
sbgj_glass_stones <- function(output = NULL, ...,
                              paper = c("letter", "a4"),
                              instructions = FALSE) {
    check_dots_empty()
    assert_runtime_dependencies()
    ...
    if (!file.exists(pic))
        download.file("https://i2.pickpik.com/photos/301/542/392/glass-beads-glass-blue-decoration-a9ef02658d94bb22dd25203b4aa9f858.jpg",
        pic)
...
#' @rdname sbgj_storage
#' @export
sbgj_pawns <- function(output = NULL, ...,
                       paper = c("letter", "a4"),
                       instructions = FALSE) {
    check_dots_empty()
    assert_runtime_dependencies()
    ...
    credits <- c("* *Play stone, Multicoloured, Characters* by Alexa",
                 "",
                 "  + https://pixabay.com/photos/play-stone-multicoloured-characters-1743645/",
...
#' @rdname sbgj_storage
#' @export
sbgj_polyhedral_dice <- function(output = NULL, ...,
                                 paper = c("letter", "a4"),
                                 instructions = FALSE) {
    check_dots_empty()
    assert_runtime_dependencies()
    ...
    if (!file.exists(pic))
        download.file("https://i2.pickpik.com/photos/821/950/430/cube-play-role-playing-game-craps-82caba6d073e803940fba76a77c4be8a.jpg",
        pic)
    bm_pic <- magick::image_read(pic) |> as_bm_pixmap()
    nc <- ncol(bm_pic)
    bm_front <- rasterGrob(bm_pic[, seq.int(ceiling(nc / 2) - 24L, nc)], height = 1)
    ...
    df <- ppdf::dice_dice(x = rep(1:4 - 0.5, 7L),
                          ...
                          rank = rep(1:4, 7L),
                          suit = rep(6:1, length.out = 28L)) |>
            mutate(cfg = rep(paste0("dice_", c("d4", "numeral", "d8", "d10", "d10_percentile", "d12", "d20")),
                             each = 4L))
    dice <- piecepackr::pmap_piece(df, piecepackr::pieceGrob,
                                   scale = 0.88, envir = envir, op_scale = 0.01,
                                   draw = FALSE, default.units = "in")
    ...
    spine <- gList(rectGrob(gp = gpar(col = NA, fill = "black")),
                   spineTextGrob("Polyhedral Dice"))
    xmp <- xmp(creator = "Trevor L. Davis", date_created = "2025",
               spdx_id = "CC-BY-ND-4.0",
               title = "Polyhedral Dice Small Box Game Jacket")

    credits <- c("* *assorted dice lot*",
                 "",
